...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 079 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。
...
```text
同一性・構造・空間・動作・関係を抽出 → 直線、折れ線、幾何軸で骨格化 → 有機曲線と探り線で崩す → 原写真の識別色を透明淡彩で点染 → 背景を削り広い紙白を残す → 構造線と余白に沿って少量の文字を配置
```

## 完成作の特徴

- 線は細く不均一で、探り線、断線、ずれ、未完の縁を残す。
- 淡彩は元写真の識別色だけを透明に置き、紙白を広く残す。
- 複雑な背景を削り、空間と距離を余白に担わせる。
- 偏置、部分的な裁ち落とし、片側への伸長により、広い紙白が空、空気、距離、光、間を担う。
- 細い編集書体は透視線、軸、被写体の縁、余白に沿い、定型のタイトル＋本文にはしない。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-079-prompt.en.md)
...
79 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 079 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–079） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
