...
上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 079 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

一般的な写真輪郭フィルターは縁を機械的に複製するだけです。一方、完成した水彩風景は背景を描き込みすぎて、被写体、空間、編集のリズムを一緒に重くしがちです。

**Panel 079** は主体、空間、動作、関係を強い直線と自由な有機曲線で再構成し、原写真から少量の透明淡彩だけを加えます。広い紙白と未完成の探り線が、建築旅行スケッチの秩序と現場の手触りを両立します。

### 向いている用途

- 建築、旅行、現場観察を、最も力のある直線、折れ、切断面、比例関係で空間化したい場合
- 探索的な重複線、断線、わずかなずれ、未完の縁を受け入れ、「描きかけ」の生命力を残したい場合
- 色を写真から取りながら塗り広げず、必要な場所だけに透明淡彩を置いて、インク線、色、紙白を階層化したい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 079 で処理し、最適な構図とサイズを先に提案してください。
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 建築スケッチの骨格、現場描写のゆるさ、独立誌の組版を併せ持つ観察ページ |
| **識別しやすい特徴** | 強い直線骨格を自由曲線が崩し、少量の淡彩が広い紙白に浮かぶ |
| **入力の尊重** | 同一性、輪郭、姿勢、空間、物語関係を保ち、色は元写真の識別色だけから導く |
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-079`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-079-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-079-prompt.zh-CN.md)
...
**特徴：** 強い幾何直線 · 自由な有機曲線 · ペン淡彩 · 未完成感 · 広い紙白 · 編集的な図文構成
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 079 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-079 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP

# 左右比較：3:2 横長、文字なし
/xxd-panel-079 photo.jpg --mode left-right --size 3:2 --text none

# デザインのみ：9:16 スマートフォン縦長、日本語文字
/xxd-panel-079 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP
...
/xxd-panel-079 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-079 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-079 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 079 を適用し、共通設定は一度だけ解決
/xxd-panel-079 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
XXD Panel の現行シリーズは 001〜112 まで更新され、各 Panel は独立した原文プロンプトと美的ロジックを保持しています。以下は本プロジェクト公開時点の履歴カタログで、001〜079 を連続して掲載し、現在のプロジェクトを太字で示します。
