...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 079 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。

<!-- xxd-human-intro:start -->
## 适用场景与解决的问题

普通照片描边滤镜只会机械复制边缘；完整水彩又容易画满背景，让主体、空间和编辑节奏一起变重。

**Panel 079** 只保留照片中最值得记住的结构、空间、动作与现场关系：长直透视线、折线和几何轴线建立理性骨架，自由有机曲线、寻找线与断线保留手感，少量透明水彩从原图提取辨识色。大片自然纸白承担空气与停顿，文字沿结构线和留白参与构图，成品像经过艺术指导的建筑旅行速写本或独立视觉刊物。
...
> 用 XXD Panel 079 帮我处理这张图，先推荐最合适的构图和尺寸。
...
**关键词：** 强几何直线 · 自由有机曲线 · 钢笔淡彩 · 未完成感 · 大面积自然纸白 · 编辑式图文混排

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 079 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅兼具建筑速写的结构秩序、现场手绘的松弛与独立刊物排版的观察页 |
| **一眼可辨的特点** | 强直线骨架与自由曲线相互牵制，少量透明淡彩悬在大片近白纸面上 |
| **它如何尊重你的输入** | 保留最关键的身份、轮廓、姿态、空间和叙事关系，颜色只从原图的辨识色中提取 |
...
```text
提取身份、结构、空间、动作与叙事关系 → 用长直线、折线与几何轴线建立骨架 → 以自由有机曲线和寻找线打破秩序 → 从原图提取少量辨识色进行透明点染 → 删除复杂背景并保留大片纸白 → 让纤细文字沿结构线与留白参与构图
```

## 成品中最容易识别的风格特点

- 远看有清晰几何秩序，近看能看到松散、不均匀的探索线、断线与未完成边缘。
- 建筑和器物优先抓直线、转折、切面与比例；人物、植物、动物、云和水以有机曲线打破骨架。
- 透明水彩只在必要位置轻薄点染，可轻微越线并保留水痕、颗粒与覆盖不均。
- 构图允许偏置、局部裁切和向一侧延伸，大片纸白承担天空、空气、距离与停顿。
- 文字使用纤细编辑字体，沿透视线、几何轴线、主体边缘或留白展开，不做菜单式标题正文。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-079-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 079 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-079 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 模型依照原始提示词决定文字，只限定语言
/xxd-panel-079 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-079 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-079 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 079；公共设置只解析一次
/xxd-panel-079 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-079`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-079-prompt.en.md) · [中文运行适配器](references/xxd-panel-079-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全系列项目

79 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–079 的全部项目地址和核心风格特点；当前项目以粗体标出。
...
| **士兵级** | `xxd-panel-NNN`（当前 001–079） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
