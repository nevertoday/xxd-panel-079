...
# 🦁 XXD Panel 079｜펜 담채 저널

### 사진을 기하 골격·자유 잉크선·담채가 있는 현장 관찰 페이지로 바꿉니다
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 079 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 079 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 079 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 079 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 079 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 079 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 079 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 079 additional top-bottom sample 4"></td>
  </tr>
</table>
...
위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 079이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

흔한 사진 외곽선 필터는 가장자리를 기계적으로 복제할 뿐이고, 완성형 수채 풍경은 배경을 가득 그려 피사체, 공간과 편집 리듬을 함께 무겁게 만들기 쉽습니다.

**Panel 079**는 피사체·공간·행동·관계를 강한 직선과 자유로운 유기 곡선으로 재구성하고, 원본에서 고른 소수의 투명 담채만 더합니다. 넓은 종이 흰색과 미완성 탐색선이 건축 여행 스케치의 질서와 현장 손맛을 함께 만듭니다.

### 이런 경우에 적합합니다

- 건축, 여행과 현장 관찰을 가장 힘 있는 직선, 꺾임, 절단면과 비례 관계로 공간화하고 싶을 때
- 탐색적인 중복선, 끊긴 선, 가벼운 어긋남과 미완성 가장자리를 받아들여 ‘그리다 만’ 생명력을 남기고 싶을 때
- 색은 사진에서 가져오되 넓게 칠하지 않고 필요한 곳에만 투명 담채를 사용해 잉크선, 색과 종이 흰색을 층위화하고 싶을 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 079로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 건축 스케치의 구조, 현장 드로잉의 느슨함, 독립 출판물 타이포그래피를 결합한 관찰 페이지 |
| **대표 특징** | 강한 직선 골격을 자유 곡선이 깨뜨리고, 소량의 담채가 넓은 종이 흰색 위에 떠 있음 |
| **입력 존중 방식** | 정체성, 윤곽, 자세, 공간과 서사 관계를 보존하고 색은 원본의 식별색에서만 가져옴 |
...
```bash
git clone https://github.com/nevertoday/xxd-panel-079.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-079" ~/.codex/skills/xxd-panel-079
```

`npx skills`로도 바로 설치할 수 있습니다:

```bash
npx skills add https://github.com/nevertoday/xxd-panel-079 --skill xxd-panel-079
```
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-079`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-079-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-079-prompt.zh-CN.md)
...
**특징:** 강한 기하 직선 · 자유 유기 곡선 · 펜 앤 워시 · 미완성 감각 · 넓은 종이 흰색 · 편집형 이미지·타입 구성
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 079의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
```text
/xxd-panel-079 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-079 photo.jpg --prefs last
/xxd-panel-079 photo.jpg --prefs edit
/xxd-panel-079 photo.jpg --prefs new
```
...
/xxd-panel-079 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-079 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-079 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR

# 연결형 4종 배경화면: 기준 이미지 후 기기별 재구성
/xxd-panel-079 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# 독립형 배경화면: 기기별 정확한 해상도 지정
/xxd-panel-079 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# 원본 이미지 비율 따르기
/xxd-panel-079 photo.jpg --mode design-only --size source --text none

# 정사각형·세로·가로 완성 구도를 각각 생성
/xxd-panel-079 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# 사용자 비율과 정확한 픽셀을 함께 지정
/xxd-panel-079 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# 원본 프롬프트가 문구를 만들고 언어만 제한
/xxd-panel-079 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-079 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-079 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 079 적용, 공통 설정은 한 번만 확정
/xxd-panel-079 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR

# 반복 매개변수도 누적하며 지정 루트 아래로 출력
/xxd-panel-079 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
현재 XXD Panel 시리즈는 001–112까지 업데이트되었으며, 각 Panel은 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표는 이 프로젝트 공개 시점의 과거 목록으로 001–079을 연속해서 나열하며, 현재 프로젝트는 굵게 표시합니다.
