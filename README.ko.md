...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 079이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.
...
```text
정체성·구조·공간·행동·관계 추출 → 직선·꺾임·기하 축으로 골격화 → 유기 곡선과 탐색선으로 질서 깨기 → 원본의 식별색을 투명 담채로 점염 → 복잡한 배경을 지우고 넓은 종이 흰색 유지 → 구조선과 여백을 따라 소량의 글자 배치
```

## 완성작의 특징

- 선은 가늘고 고르지 않으며 탐색선, 끊김, 어긋남과 미완성 가장자리를 남깁니다.
- 담채는 원본의 식별색만 투명하게 사용하고 종이 흰색을 넓게 남깁니다.
- 복잡한 배경을 삭제하고 공간과 거리를 여백이 맡게 합니다.
- 치우친 배치, 부분 크롭과 한쪽으로의 확장으로 넓은 종이 흰색이 하늘, 공기, 거리, 빛과 멈춤을 담당합니다.
- 가는 편집 서체는 원근선, 축, 피사체 가장자리나 여백을 따르며 정형화된 제목+본문 구성을 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-079-prompt.en.md)
...
79개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 079까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
...
| **병사급** | `xxd-panel-NNN` (현재 001–079) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
